...
To allow logged-out users to access some Superset features, you can configure the **Public** role with specific permissions. This gives you fine-grained control over what anonymous users can access.

#### Configuring the Public Role for Dashboard Access

To enable public dashboard access:

1. Enable the `DASHBOARD_RBAC` feature flag in your `superset_config.py`:
   ```python
   FEATURE_FLAGS = {
       "DASHBOARD_RBAC": True,
   }
...
2. In the Superset UI, navigate to **Settings** > **List Roles** and edit the **Public** role.

3. Add the following permissions to the **Public** role:
...
4. Assign the **Public** role to specific dashboards as described in the [dashboard documentation](/docs/using-superset/creating-your-first-dashboard/#manage-access-to-dashboards).

5. Grant access to specific datasets: In addition to dashboard-level permissions, you must also grant the **Public** role access to the datasets used by your public dashboards. Navigate to **Settings** > **List Roles**, edit the **Public** role, and add the relevant dataset permissions in the **Permissions** dropdown.

With this configuration, anonymous users can view dashboards that have been explicitly assigned the **Public** role.

#### Using PUBLIC_ROLE_LIKE (Simpler Alternative)

As a simpler alternative, you can use the `PUBLIC_ROLE_LIKE` config setting to copy permissions from another role, though this offers less control and may grant more permissions than necessary.

For example, by setting `PUBLIC_ROLE_LIKE = "Gamma"` in your `superset_config.py` file, you grant
the public role the same set of permissions as the **Gamma** role. Explicit grant on specific datasets is
